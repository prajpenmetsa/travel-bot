...
class EvaluationService:
    ...
    def _budget_realism(self, nar, budget) -> float:
        ...
        # Get the calculated total from budget in a robust way
        if isinstance(budget, dict) and "total" in budget:
            if isinstance(budget["total"], dict) and "grand_total" in budget["total"]:
                calc_total = budget["total"]["grand_total"]
            elif isinstance(budget["total"], (int, float)):
                calc_total = budget["total"]
        elif isinstance(budget, (int, float)):
            calc_total = budget
        else:
            return 0.3  # Default score if we can't find a budget total
